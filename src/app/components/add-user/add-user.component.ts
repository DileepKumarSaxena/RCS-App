...
function noSpacesValidator(control: AbstractControl): { [key: string]: boolean } | null {
...
export class AddUserComponent {
  ...
  ngOnInit() {
    this.addUserForm = this.formBuilder.group({
      ...
      botId: ['', [Validators.required, Validators.minLength(3), Validators.maxLength(100), noSpacesValidator]],
      botToken: ['', [Validators.required, Validators.minLength(3), Validators.maxLength(200), noSpacesValidator]],
  ...
  getCampaignInfo() {
    this.userservice.edituserData(this.userId).subscribe({
      next: (res: any) => {
        const user = res.USER; //Extract the USER object from the response
        ...
        // this.addUserForm.controls['phoneNumber'].disable();
        this.addUserForm.controls['Username'].disable();
      },
      error: (err) => {
        if(err.status=500){
          Swal.fire('Something Went Wrong!!')
          this.router.navigate(['/user/list'], { queryParams: { } });
        }
  ...
  onSubmit() {
    ...
    if (this.userId) {
      ...
      this.userservice.updateUserDetails(requestData).subscribe({
        next: (res) => {
          this.loading = false;
          Swal.fire({
        ...
        error: () => {
          this.loading = false; // Set loader to false on error
          Swal.fire({
            title: 'Error',
      ...
      // Add a new user
      this.userservice.addNewUser(requestData).subscribe({
        next: (res) => {
